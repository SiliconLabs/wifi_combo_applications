...
#define RSI_REMOTE_DEVICE_NAME "Remote_device"

#define RSI_BLE_APP_DEVICE_NAME "917_ble"
...
sl_wifi_performance_profile_t wifi_profile1 = { .profile = ASSOCIATED_POWER_SAVE_LOW_LATENCY };
#endif
...
#define RSI_APP_EVENT_POWER_SAVE 3
...
void ble_app_task()
{
  ...
  status = sl_wifi_set_performance_profile(&wifi_profile1);
  ...
  while (1) {
      ...
      switch (temp_event_map) {
        case RSI_APP_EVENT_ADV_REPORT: {
          ...
          if ((temp_event_map1 == -1) || (!(temp_event_map1 & RSI_APP_EVENT_CONNECTED))) {
              ...
              if (status != RSI_SUCCESS) {
                  ...
                  rsi_ble_app_set_event(RSI_APP_EVENT_POWER_SAVE);
        ...
        case RSI_APP_EVENT_CONNECTED: {
          ...
          osDelay(5000);
          rsi_ble_disconnect((int8_t*)rsi_app_connected_device.dev_addr);
        ...
        case RSI_APP_EVENT_DISCONNECTED: {
          ...
          //! initiating power save in wlan mode
          wifi_profile1.profile = HIGH_PERFORMANCE;
          status               = sl_wifi_set_performance_profile(&wifi_profile1);
          if (status != SL_STATUS_OK) {
          ...
          //! initiating power save in wlan mode
          wifi_profile1.profile = ASSOCIATED_POWER_SAVE;
          status               = sl_wifi_set_performance_profile(&wifi_profile1);
          if (status != SL_STATUS_OK) {
