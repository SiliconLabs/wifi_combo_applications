...
#define PUBLISH_PERIODICITY       osWaitForever //! Publish periodicity in milliseconds
...
#define LOW                    0
#define COEX_SSID         "Silabs_AP"
#define COEX_SECURITY_TYPE 2   // 0-Open, 1-WPA, 2-WPA2
#define COEX_PSK          "12345678"
/*
 *********************************************************************************************************
...
char ta_active=1,ta_unconnected=2,ta_connected=3;
...
sl_status_t TA_Power_save(char ta_state)
{
  sl_status_t status_ps=0;
  ...
  if(ta_state==1){
      ...
      rsi_bt_power_save_profile(PSP_MODE, PSP_TYPE);
      if (status_ps != RSI_SUCCESS) {
  ...
  else if(ta_state == 3)
    {
      ...
      performance_profile .listen_interval = 1000;
      rsi_bt_power_save_profile(PSP_MODE, PSP_TYPE);
      if (status_ps != RSI_SUCCESS) {
          LOG_PRINT("\r\n TA Failed to initiate power save in BLE mode \r\n");
          return status_ps;
  ...
  while (1) {
      ...
      switch (event_id) {
        ...
        case WIFI_APP_SCAN_STATE: {
          ...
          M4_Power_save(); //Waits till the BTN0 press
        ...
        case WIFI_APP_DISCONN_NOTIFY_STATE: {
          ...
          if (status == RSI_SUCCESS) {
              ...
              status= TA_Power_save(ta_unconnected);
              M4_Power_save();
