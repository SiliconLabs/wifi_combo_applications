...
/* Mqtt topics */
//! Subscribe Topic
#define MQTT_TOPIC1               "Temperature"   //! Subscribe Topic to receive the message from cloud
//! Publish Topic
#define MQTT_TOPIC2               "EFRtemp" //! Publish Topic to send the status from application to cloud
...
sl_wifi_performance_profile_t wifi_profile = { .profile = ASSOCIATED_POWER_SAVE, .listen_interval = 1000 };
...
extern float efr_temp;
extern uint8_t bt_role;
extern int8_t provisioner_bd_addr[6];
...
void async_socket_select(fd_set *fd_read, fd_set *fd_write, fd_set *fd_except, int32_t status1)
{
  ...
  sl_wifi_app_set_event(SL_AWS_SELECT_CONNECT_STATE);
}
  ...
  while (1) {
    ...
    switch (event_id) {
      ...
      case SL_WIFI_IPCONFIG_DONE_STATE: {
        ...
        rsi_ble_disconnect(provisioner_bd_addr);
      } break;
      ...
      case SL_WLAN_SET_MQTT: {
        sl_wifi_app_clear_event(SL_WLAN_SET_MQTT);
  ...
  while (1) {
    ...
    switch (sl_wlan_app_cb.state) {
      ...
      case SL_WIFI_MQTT_SUBSCRIBE_STATE: {
        ...
        sl_wlan_app_cb.state = SL_AWS_SELECT_CONNECT_STATE;
      ...
      case SL_AWS_SELECT_CONNECT_STATE: {
        {
          LOG_PRINT("\r\n SL_AWS_SELECT_CONNECT_STATE..\n");
     ...
     if (NETWORK_ATTEMPTING_RECONNECT == rc) {
        ...
        if ((!publish_timer_start) || publish_msg) {
          ...
          LOG_PRINT("\r\nSL_WIFI_MQTT_PUBLISH_STATE..\n");
          sprintf(cpayload, "{\"status\":  \"Current temperature in °C\"  ,\"value\": %f }", efr_temp);
          LOG_PRINT("\r\n Current temperature in °C : %f\r\n ", efr_temp);
          publish_QOS0.qos        = QOS0;
          publish_QOS0.payload    = (void *)cpayload;
          publish_QOS0.isRetained = 0;
          publish_QOS0.payloadLen = strlen((char *)cpayload);
        ...
        sl_wlan_app_cb.state = SL_AWS_SELECT_CONNECT_STATE;
#if ENABLE_POWER_SAVE
        sl_wlan_app_cb.state = SL_SLEEP_STATE;
#endif
...
#if ENABLE_POWER_SAVE
      case SL_SLEEP_STATE: {
        sl_si91x_host_delay_ms(SLEEP_PERIODICITY);
#ifdef SLI_SI91X_MCU_INTERFACE
