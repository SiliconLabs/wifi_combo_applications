...
/// Wi-Fi client configuration for CLI Demo
...
/// Wi-Fi AP configuration for CLI Demo
...
/// Wi-Fi concurrent (AP + STATION) configuration for CLI Demo
...
/// Wi-Fi enterprise client configuration for CLI Demo
...
/// Wi-Fi transmit configuration for CLI Demo
...
sl_status_t sl_wifi_update_gain_table_command_handler(console_args_t *arguments)
{
  ...
  uint8_t gain_table_payload[] = { 3,  0,  13, 1, 34, 20, 20, 22, 2,  34, 28, 28, 28,  3,  34,  32, 32, 32,  4,  34,
                                   36, 36, 34,  5, 34, 38, 38, 38,  6, 34, 40, 40, 38,  7,  34, 38,  38, 38,  8,  34, 36,
                                   36, 38, 9,  34, 32, 32, 32, 10, 34, 32, 32, 28, 11, 34, 24, 24, 24, 12, 34, 16, 24, 24,
                                   13, 34, 12, 12, 12, 2,  17, 255, 20, 16, 16, 4, 17, 255, 26, 20, 20 };
...
sl_status_t set_region_configuration_handler(console_args_t *arguments)
{
  ...
  status = sl_si91x_set_device_region(set_region_config.operation_mode,
                                      set_region_config.band,
                                      set_region_config.region_code);
  VERIFY_STATUS_AND_RETURN(status);
  return SL_STATUS_OK;
}