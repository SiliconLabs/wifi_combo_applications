...
silabs_image = resource_path("silabs.png")
...
def msgbox(inp1,inp2):                                      # Error message box
    ...
    if(opersys == 'Darwin'):
        button = Button(roott,width=130, text="OK", command=lambda: roott.destroy()).pack(pady=20)
    else:
...
def interface():                                            # Window for selecting port and interface
    ...
    if(opersys == 'Darwin'):
        Label(root, text="",font=("Verdana 8 bold",15),command=None,bg="#023242",fg="white").pack(pady=0)
        Label(root,text="Select an Interface",height=2,width=27,font=("Verdana 8 bold",15),command=None,bg="#023242",fg="white").pack(pady=0)
    else:
        Label(root, text="",font=("Verdana 8 bold",15),command=None,bg="#023242",fg="white").pack(pady=0)
        Label(root, text="Select an Interface",font=("Verdana 8 bold",15),command=None,bg="#023242",fg="white").pack(pady=0)
    ...
    if(opersys == 'Darwin'):
        menu.config(bg="#E0E6F2", fg="#4A5264",width=20)
        menu.pack(pady=10, ipadx=20)
    else:
        menu.config(bg="#023242", fg="white",width=20)
        menu.pack(pady=20, ipadx=20)
    if(opersys == 'Darwin'):
        Label(root,text="Select a Port",height=2,width=27,font=("Verdana 8 bold",15),command=None,bg="#023242",fg="white").pack(pady=5)
    else:
        Label(root, text="Select a Port",font=("Verdana 8 bold",15),command=None,bg="#023242",fg="white").pack(pady=0)
    portclicked.set(module_ports[0])
    portmenu = OptionMenu( root , portclicked , *module_ports )
    if(opersys == 'Darwin'):
        portmenu.config(bg="#E0E6F2", fg="#4A5264",width=20)
        portmenu.pack(pady=10, ipadx=20)
    else:
        portmenu.config(bg="#023242", fg="white",width=20)
        portmenu.pack(pady=20, ipadx=20)
    
    if(opersys == 'Darwin'):
        button = Button(root, text= "OK", width= 130,command=lambda: show(root,clicked,portclicked)).pack(pady=10)
    else:
...
main=Tk()
main.title("	Wi-Fi Provisioning over BLE",)
screen_width = main.winfo_screenwidth()
screen_height = main.winfo_screenheight()

if opersys=='Darwin':
    main.geometry("{width}x{height}".format(width=screen_width,height=screen_height))
else:
...
if opersys=='Darwin':
    canvas = Canvas(container,width=screen_width/2,height=3*(screen_height*2/9),bd=3,background="black")
else:
...
if opersys=='Darwin':
    exit_button = Button(main, text="Exit", command=main.destroy,height=40,width=130,bg="red",fg="white",font='Verdana 11 bold')
else:
    exit_button = Button(main, text="Exit", command=main.destroy,height=2,width=20,bg="red",fg="white",font='Verdana 11 bold')
exit_button.pack(pady=10)

if opersys=='Darwin':
    restart_button = Button(main, text="Restart", command=restart_program, height=40,width=130,bg="green",fg="white",activebackground="#0000ff",font='Verdana 11 bold')
else:
    restart_button = Button(main, text="Restart", command=restart_program, height=2,width=20,bg="green",fg="white",font='Verdana 11 bold')
restart_button.pack(pady=5)

def framelabel(scrollable_frame,inp) :
    if opersys=='Darwin':
    	lbl_mac=Label(scrollable_frame,text=inp,bd=2,height=3,width=int(screen_width/14),font=("Verdana 7 bold",10),command=None,bg="#023242",fg="white", relief=GROOVE)
    else: 
...
def band():
    ...
    if(len(response)>0):
        if(len(response)>=2 and response[0:2].decode(errors="ignore")=='OK'):
            print("Band set\n")
            framelabel(scrollable_frame,"Band set to 2.4 GHz")
        else:   
    ...
    if(len(response) > 0):
        ...
        if len(response2)>0 :
            if(len(response2[0])>=2 and response2[0][0:2].decode(errors="ignore")=='OK'):
                ...
                framelabel(scrollable_frame,"Number of APs = " + str(r))
                if r>0:
                    i=0        
                    for listitr in SSID :
                        ...
                        for inditr in range(len(listitr)):
                            apname = apname + bytes.fromhex(listitr[inditr]).decode("ASCII") 
                        ...
                        if len(response1)>0 :
                            if(len(response1[0])>=2 and response1[0][0:2].decode(errors="ignore")=='OK'):
                                ...
                                framelabel(scrollable_frame,"AP " + str(i) +": "+ str(apname))
                                main.update()
                            else:
