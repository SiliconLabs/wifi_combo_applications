    ...
    case RSI_WLAN_REQ_JOIN:
      {
        ...
        memcpy(rsi_join->join_bssid, wlan_cb_non_rom.join_bssid_non_rom,6);
    ...
    case RSI_WLAN_REQ_SOCKET_CREATE:
      {
        ...
        uint8_t sock_id = rsi_driver_cb->wlan_cb->waiting_socket_id;
        socket_create->vap_id  = rsi_socket_pool_non_rom[sock_id].vap_id;
        ...
        if(rsi_socket_pool_non_rom[sock_id].max_tcp_retries)
        {
          socket_create->max_tcp_retries_count  = rsi_socket_pool_non_rom[sock_id].max_tcp_retries;
        }
        else
        {
          socket_create->max_tcp_retries_count  = 10;
        }
        if(rsi_socket_pool_non_rom[sock_id].tcp_keepalive_initial_time)
        {
          memcpy(socket_create->tcp_keepalive_initial_time,rsi_socket_pool_non_rom[sock_id].tcp_keepalive_initial_time,2);
        }
        else
        {
        ...
        if(rsi_socket_pool_non_rom[sock_id].ssl_bitmap)
        {
          socket_create->ssl_bitmap  = rsi_socket_pool_non_rom[sock_id].ssl_bitmap;
        }
        else if(socket_create->socket_bitmap & RSI_SOCKET_FEAT_SSL)
        {
          socket_create->ssl_bitmap   =  wlan_cb_non_rom.tls_version;
        }
        
        if(rsi_socket_pool_non_rom[sock_id].high_performance_socket)
        {
...
{
  uint8_t  cmd_type;
  uint8_t  ip_version,sockID,sock_id,accept_sock_id;
  int32_t  status = RSI_SUCCESS;
...
if(global_cb_p->os_enabled == 1) {
  ...
  switch(cmd_type)
  {
  	 ...
  	 case RSI_WLAN_RSP_CONFIRM:
  	  	  {
  	  			...
  	  			{
  	  				if(rsi_driver_cb->wlan_cb->callback_list.flash_write_response_handler) {
  	  					rsi_driver_cb->wlan_cb->callback_list.flash_write_response_handler(status);
  	  				}
  	  				return RSI_SUCCESS;
    ...
    case RSI_WLAN_RSP_SELECT_REQUEST:
      {
        rsi_fd_set *fd ;
        //! taking readfd address from fd_set_ptr
        fd = (rsi_fd_set *)wlan_cb_non_rom.fd_set_ptr;
        if(payload_length == 0)
        {
          //!set fd_count to zero to indicate time out error
          fd->fd_count = 0;
        }
        else
        {
          ...
          RSI_FD_ZERO(fd);
          fd->fd_count = 0;
          //!set the readfd with the updated value from firmware response
        ...
        if(wlan_cb_non_rom.sock_select_callback != NULL)
        {
          wlan_cb_non_rom.sock_select_callback((uint32_t )wlan_cb_non_rom.fd_set_ptr);
        }
    ...
    case RSI_WLAN_RSP_SOCKET_CREATE:
      {
#ifndef RSI_M4_INTERFACE
        if(rsi_wlan_cb->callback_list.socket_connect_response_handler != NULL)
        {
          //! Call asynchronous response handler to indicate to host
          rsi_wlan_cb->callback_list.socket_connect_response_handler(status, payload, payload_length);
        }
#endif
        if(status == RSI_SUCCESS)
        {
	...
	case RSI_WLAN_RSP_URL_REQUEST:
      {
        ...
        if(rsi_driver_cb->nwk_cb->nwk_callbacks.rsi_webpage_request_handler != NULL)
        {
          if(status == RSI_SUCCESS)
          {
            //! Call asynchronous response handler to indicate to host
        ...
        if(rsi_driver_cb->nwk_cb->nwk_callbacks.rsi_http_client_put_response_handler != NULL)
        {
          if(status == RSI_SUCCESS)
          {
            ...
            //! Call HTTP client PUT asynchronous response handler to indicate to host
	          rsi_driver_cb->nwk_cb->nwk_callbacks.rsi_http_client_put_response_handler(status, (uint8_t)*payload, payload+RSI_HTTP_PUT_OFFSET, payload_length, end_of_file);
          }
          else
          {
            //! Call POP3 client asynchronous response handler to indicate to host
            rsi_driver_cb->nwk_cb->nwk_callbacks.rsi_http_client_put_response_handler(status, host_desc[5], payload+RSI_HTTP_PUT_OFFSET, payload_length, 0);
          }
    ...
    case RSI_WLAN_RSP_UART_DATA_ACK:
      {
        if(!(rsi_socket_pool[wlan_cb_non_rom.rsi_uart_data_ack_check].sock_bitmap & RSI_SOCKET_FEAT_TCP_ACK_INDICATION))
        {        
    ...
    case RSI_WLAN_RSP_ADD_MFI_IE :
      ...
      case RSI_WLAN_RSP_SNTP_CLIENT: 
      {
        ...
        if(rsi_driver_cb->nwk_cb->nwk_callbacks.rsi_sntp_client_create_response_handler != NULL)
        {
          if(status == RSI_SUCCESS)
          { 
            if(((uint8_t)*payload == RSI_SNTP_CREATE) 
              ...
              rsi_driver_cb->nwk_cb->nwk_callbacks.rsi_sntp_client_create_response_handler(status, (uint8_t)*payload, payload);
              //! Clear expected response
              rsi_wlan_cb->expected_response = RSI_WLAN_RSP_CLEAR;
              return RSI_SUCCESS;
...
//! this switch case copy for response buffer to user space. 
	  	...
	  	case RSI_WLAN_RSP_CERT_VALID:
      {
        if(status == RSI_SUCCESS)
        {
          ...
          {
            RSI_WLAN_SET_STATUS(RSI_ERROR_INSUFFICIENT_BUFFER);
            status = RSI_ERROR_INSUFFICIENT_BUFFER;
          }
        }
        if(rsi_wlan_cb->callback_list.certificate_response_handler != NULL)
        {
          //! Call asynchronous response handler to indicate to host
          rsi_wlan_cb->callback_list.certificate_response_handler(status, payload, payload_length);
        }
      }
      break;
      case RSI_WLAN_RSP_MODULE_STATE:
      {
        if(status == RSI_SUCCESS)
        {
          if(rsi_driver_cb->wlan_cb->callback_list.wlan_async_module_state != NULL)
          {
            rsi_driver_cb->wlan_cb->callback_list.wlan_async_module_state(status, payload, payload_length);
          }
        }
      }
      break;
...
#ifdef RSI_CHIP_MFG_EN
	  (wlan_cb->expected_response == RSI_FLASH_WRITE) ||
#endif
