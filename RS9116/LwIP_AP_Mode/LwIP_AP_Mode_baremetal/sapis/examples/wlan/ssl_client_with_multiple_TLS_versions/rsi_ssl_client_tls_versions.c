...
int32_t rsi_ssl_client()
{
  ...
  client_socket2 = socket(AF_INET, SOCK_STREAM, 0);
	...
	{
		status = rsi_wlan_get_status();
		LOG_PRINT("\r\nSocket2 Create Failed, Error Code : 0x%X\r\n",status);
		return status;
	}
	else
	{
  ...
  server_addr.sin_addr.s_addr = SERVER_IP_ADDRESS;
	...
	while( 1 ) //packet_count < NUMBER_OF_PACKETS)
	{
		...
		if(status < 0)
		{
			status = rsi_wlan_get_status();
			rsi_shutdown(client_socket, 0);
			LOG_PRINT("\r\nFailed to Send data on socket, Error Code : 0x%X\r\n",status);
			return status;
		}

		//! Send data on socket
		status = rsi_send(client_socket2,(int8_t *)"Hello from SSL TCP client!!!", (sizeof("Hello from SSL TCP client!!!") - 1), 0);
		if(status < 0)
		{
			status = rsi_wlan_get_status();
			rsi_shutdown(client_socket2, 0);
			LOG_PRINT("\r\nFailed to Send data on socket2, Error Code : 0x%X\r\n",status);
			return status;
		}
...
int main()
{
  ...
  rsi_task_create(rsi_ssl_client, "wlan_task", RSI_WLAN_TASK_STACK_SIZE, NULL, RSI_WLAN_TASK_PRIORITY, &wlan_task_handle);
