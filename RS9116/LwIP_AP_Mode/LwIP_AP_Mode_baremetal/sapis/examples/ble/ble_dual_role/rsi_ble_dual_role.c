...
void rsi_ble_on_enhance_conn_status_event(rsi_ble_event_enhance_conn_status_t *resp_enh_conn)
{
  ...
  else if (!strcmp((char *)str_conn_device, RSI_BLE_DEV_8_ADDR))
  {
    ...
    for(ix = 8 ; ix <= 9 ; ix++) {
      rsi_6byte_dev_address_to_ascii ((int8_t *)str_conn_device, (uint8_t *)conn_event_to_app[ix].dev_addr);	
...
static void rsi_ble_on_disconnect_event(rsi_ble_event_disconnect_t *resp_disconnect, uint16_t reason)
{
  ...
  else if (!strcmp((char *)str_disconn_device, RSI_BLE_DEV_8_ADDR))
  {
    ...
    for(ix = 8 ; ix <= 9 ; ix++) {
      rsi_6byte_dev_address_to_ascii ((int8_t *)str_conn_device, (uint8_t *)conn_event_to_app[ix].dev_addr);	
      if (!strcmp((char *)str_conn_device, str_disconn_device)) {
...
int main (void)
{
 ...
 //! Driver initialization
  ...
  rsi_task_create(rsi_ble_dual_role, "ble_task", RSI_BT_TASK_STACK_SIZE, NULL, RSI_BT_TASK_PRIORITY, &bt_task_handle);
