...
uint8_t adv_payload_for_compare[31] = {0x6E,0xC5,0xFD,0x05,0x54,
		0x9E,0x68,0xF8,0xD0,0x05};
  ...
  uint8_t compare[31];
...
#ifdef RSI_BLE_ENABLE_WHITELIST_BASEDON_ADV_PAYLOAD
	memset(compare,0,31);
	memcpy(&compare[RSI_BLE_ADV_PAYLOAD_INDEX_FOR_COMPARE - 1], adv_payload_for_compare, RSI_BLE_ADV_PAYLOAD_LENGTH_FROM_INDEX_TO_COMPARE);
	...
	while(1)
	{
		...
		case RSI_APP_EVENT_DISCONNECTED:
		{
			...
			if(status != RSI_SUCCESS)
			{
				DEBUGOUT("start_scanning status: 0x%X\r\n", status);
			}
