...
void rsi_ble_simple_central_on_adv_report_event(rsi_ble_event_adv_report_t *adv_report)
{
  ...
  if(((device_found==0) && ((strcmp((const char *)remote_name, RSI_REMOTE_DEVICE_NAME))==0) )||
      ((remote_addr_type == RSI_BLE_REMOTE_ADDR_TYPE)&&((strcmp((const char *)remote_dev_addr,RSI_BLE_REMOTE_ADDR)==0))))
  { 
    device_found=1;
...
void rsi_ble_on_conn_update_complete_event(rsi_ble_event_conn_update_t *rsi_ble_event_conn_update_complete, uint16_t resp_status)
{
...
int32_t rsi_ble_privacy_app (void)
{
  ...
  strcpy (&adv[5], RSI_BLE_DEVICE_NAME);
	...
	if(status != RSI_SUCCESS)
	{
		LOG_PRINT("\r\nSet Scan Response Data Failed, Error Code : 0x%X\r\n",status);
		return status;
	}
	else
	{
	...
	if(status != RSI_SUCCESS)
	{
		LOG_PRINT("\r\nSet Advertise Data Failed, Error Code : 0x%X\r\n",status);
		return status;
	}
	else
	{
      ...
      case RSI_BLE_DISCONN_EVENT:
        {
          ...
          status = rsi_ble_connect(resolve_key.Identity_addr_type + 2, rsi_ascii_dev_address_to_6bytes_rev(remote_dev_bd_addr,resolve_key.Identity_addr));
          if(status != RSI_SUCCESS)
          {
            LOG_PRINT ("connect status: 0x%X\r\n", status);
          }
      ...
      case RSI_BLE_SECURITY_KEYS_EVENT:
        {
          ...
          update_resolvlist ((rsi_ble_resolvlist_group_t *)&resolvlist, &resolve_key);
			...
			//set privacy mode
			status = rsi_ble_set_privacy_mode(resolve_key.Identity_addr_type,rsi_ascii_dev_address_to_6bytes_rev(remote_dev_bd_addr,resolve_key.Identity_addr),RSI_BLE_PRIVACY_MODE);
			if(status != RSI_SUCCESS)
			{
				LOG_PRINT("line %d -> status: 0x%x\r\n", __LINE__, status);
				return status;
