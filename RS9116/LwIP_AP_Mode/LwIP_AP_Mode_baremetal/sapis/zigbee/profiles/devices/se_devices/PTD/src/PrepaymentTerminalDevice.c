...
#include "PrepaymentTerminalDevice.h"
#include "PTDevice_Application_Framework.h"
...
----------------------------------------------------------------------------*/
#if(g_PREPAYMENT_CLUSTER_ENABLE_d == 1)
uint8_t ReadMeterSerialNoValue[16];
uint8_t ReadSiteIdValue[32];
#endif
...
Alarm_Cluster_t alarm_server_cluster_values = { 0x00, 0x00 };
...
DRLCClusterAttribute_t drlc_client_cluster_default_values =
{
...
/* Application cluster definations for PrepaymentTerminal for Server */

const App_Cluster_Def_t a_Server_Cluster_List_For_PT[]=
{
#if ( g_BASIC_SERVER_CLUSTER_ENABLE_d == 1 )
...
/* Application cluster definations for Prepayment Terminal for Client */
const App_Cluster_Def_t a_Client_Cluster_List_For_PT[]=
{
...
#endif              /* ( g_TIME_CLIENT_CLUSTER_ENABLE_d == 1 ) */
   ...
   { &ga_message_Client_Cluster_Info, ZCL_Handle_Message_Cluster_Cmd, g_NULL_c, g_NULL_c  },
...
/* Device definations for Prepayment Terminal Tool for Server */
const ZCL_Device_Def_t AppDeviceListServerInfo[] =
{
    { g_PREPAYMENT_TERMINAL_ENDPOINT_c,
    (sizeof( a_Server_Cluster_List_For_PT )-1)/ sizeof( App_Cluster_Def_t ),
      a_Server_Cluster_List_For_PT },
};


/* Device definations for Prepayment Tool for Client */
const ZCL_Device_Def_t AppDeviceListClientInfo[] =
{
    { g_PREPAYMENT_TERMINAL_ENDPOINT_c,
    (sizeof( a_Client_Cluster_List_For_PT )-1)/ sizeof( App_Cluster_Def_t ),
      a_Client_Cluster_List_For_PT }
};
                ...
                if ( eventbufferindex != g_NO_AVAILABLE_BUFFER_c ) {
                    ...
                    *pLocationForResponse = g_APSDE_Data_Request_c;
                    ...
                    if( pActionHandler.action == Send_Response ) {
                        ...
                        App_updateMeterDeviceDetails(&pIndicationbuffer->a_asdu[0x03],pIndicationbuffer->asdulength-0x03);
#endif 
#endif
#endif //#if(g_SE_PROFILE_1_1_d==1)                        
    ...
    if ( status == g_ZCL_Success_c ) {
        ...
        if( rspbufferindex != g_NO_AVAILABLE_BUFFER_c ) {
            ...
            if ( eventbufferindex != g_NO_AVAILABLE_BUFFER_c ) {
                ...
                pRspLocation->asdulength = ZCL_Process_Message( g_PREPAYMENT_TERMINAL_ENDPOINT_c,
