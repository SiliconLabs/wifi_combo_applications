...
#include "IPDDevice.h"
#include "IPDDevice_Application_Framework.h"
...
const Commissioning_Cluster_t commissioning_cluster_server_default_values =
{
	...
	prepaymentAttributes_t prepayment_server_cluster_default_values;
#endif
...
const OTA_Upgrade_Cluster_Attribute_t ota_upgrade_cluster_default_values =
{
   ...
   /* Application cluster definations for IPD for Server */
const App_Cluster_Def_t a_Server_Cluster_List_For_IPD[]=
{
#if ( g_BASIC_SERVER_CLUSTER_ENABLE_d == 1 )
...
/* Application cluster definations for IPD for Client */
const App_Cluster_Def_t a_Client_Cluster_List_For_IPD[]=
{
...
/* Device definations for IPD for Server */
const ZCL_Device_Def_t AppDeviceListServerInfo[] =
{
    { g_IPD_ENDPOINT_c, ( sizeof(a_Server_Cluster_List_For_IPD)-1 )/ sizeof( App_Cluster_Def_t ),
      a_Server_Cluster_List_For_IPD },
};


/* Device definations for IPD for Client */
const ZCL_Device_Def_t AppDeviceListClientInfo[] =
{
    { g_IPD_ENDPOINT_c, ( sizeof(a_Client_Cluster_List_For_IPD)-1 )/ sizeof( App_Cluster_Def_t ),
      a_Client_Cluster_List_For_IPD },
};
                ...
                if ( eventbufferindex != g_NO_AVAILABLE_BUFFER_c ) {
                    ...
                    *pLocationForResponse = g_APSDE_Data_Request_c;
                    ...
                    case g_Cancel_Message_Events_c :
                        ...
                        break;
#endif                              /* g_MESSAGING_CLIENT_CLUSTER_ENABLE_d */
    ...
    if ( status == g_ZCL_Success_c ) {
        ...
        if( rspbufferindex != g_NO_AVAILABLE_BUFFER_c ) {
            ...
            if ( eventbufferindex != g_NO_AVAILABLE_BUFFER_c ) {
                ...
                pRspLocation->asdulength = ZCL_Process_Message( g_IPD_ENDPOINT_c,
                ...
                if( pActionHandler.action == Send_Response ) {
                    if ( (*( pRspLocation->a_asdu + g_ZCL_HEADER_COMMAND_OFFSET_c ) == g_Default_Response_Command_c ) &&
                         ( ( pApsdeDataInd->dstaddrmode == g_Short_Address_Mode_c ) &&
                           ( pApsdeDataInd->dstAddress.short_address >= g_BROADCAST_ADDRESS_FOR_ZC_AND_ZR_c ) ) ) {
                            buffMgmt_freeBuffer( rspbufferindex, 224 );
                    }
                    else {
