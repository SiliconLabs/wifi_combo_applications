...
#include "IAS_ACEDevice_Application_Framework.h"
...
*----------------------------------------------------------------------------*/
const cluster_id_t a_In_Cluster_List_For_IAS_ACE[] = {
...
const cluster_id_t a_Out_Cluster_List_For_IAS_ACE[] = {
#if ( g_SIMPLE_METERING_CLIENT_CLUSTER_ENABLE_d == 1 )
    g_SIMPLE_METERING_CLUSTER_c
#endif
#if ( g_IAS_ACE_CLIENT_CLUSTER_ENABLE_d == 1)
   g_IAS_ACE_CLUSTER_c,   /* g_SIMPLE_METERING_CLUSTER_ENABLE_d */
#endif
#if ( g_IDENTIFY_CLIENT_CLUSTER_ENABLE_d == 1)
   g_IDENTIFY_CLUSTER_c   /* g_SIMPLE_METERING_CLUSTER_ENABLE_d */
#endif	
...
{	
    ...
    sizeof(a_In_Cluster_List_For_IAS_ACE) / sizeof(cluster_id_t),	
	(cluster_id_t*)a_In_Cluster_List_For_IAS_ACE,
	sizeof(a_Out_Cluster_List_For_IAS_ACE) / sizeof(cluster_id_t),
	(cluster_id_t*)a_Out_Cluster_List_For_IAS_ACE
};

/*---------------------------------------------------------------------------*/
Endpoint_Description_t  ga_Endpoint_Descriptors[g_MAX_NO_OF_ENDPOINTS_c] =
{
	{(Simple_Descriptor_t*)&IAS_Zone_Simple_Desc, g_IAS_ACE_ENDPOINT_c },
};
