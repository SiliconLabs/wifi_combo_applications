...
#include "ApplicationThinLayer.h"
	...
	#include "IAS_ZoneDevice_Application_Framework.h"
#endif
...
*----------------------------------------------------------------------------*/
const cluster_id_t a_In_Cluster_List_For_IAS_Zone[] = {
#if ( g_BASIC_SERVER_CLUSTER_ENABLE_d == 1 )
   g_BASIC_CLUSTER_c,
#endif
#if ( g_IDENTIFY_SERVER_CLUSTER_ENABLE_d ==  1 )
...
#endif
#if ( g_POWER_SERVER_CONFIGURATION_CLUSTER_ENABLE_d == 1 )
...
#if ( g_IAS_ZONE_SERVER_CLUSTER_ENABLE_d == 1)
   g_IAS_ZONE_CLUSTER_c,   /* g_SIMPLE_METERING_CLUSTER_ENABLE_d */
#endif
#if ( g_SIMPLE_METERING_SERVER_CLUSTER_ENABLE_d == 1 )
...
const cluster_id_t a_Out_Cluster_List_For_IAS_Zone[] = {
...
const Simple_Descriptor_t IAS_Zone_Simple_Desc =
{	
	g_APP_PROFILE_ID_c,
	g_IAS_ZONE_c,
	g_APS_DEVICE_VERSION_FLAGS_c,
    sizeof(a_In_Cluster_List_For_IAS_Zone) / sizeof(cluster_id_t),	
	(cluster_id_t*)a_In_Cluster_List_For_IAS_Zone,
	sizeof(a_Out_Cluster_List_For_IAS_Zone) / sizeof(cluster_id_t),
	(cluster_id_t*)a_Out_Cluster_List_For_IAS_Zone
};

/*---------------------------------------------------------------------------*/
Endpoint_Description_t  ga_Endpoint_Descriptors[g_MAX_NO_OF_ENDPOINTS_c] =
{
	{(Simple_Descriptor_t*)&IAS_Zone_Simple_Desc, g_IAS_ZONE_ENDPOINT_c },
};
