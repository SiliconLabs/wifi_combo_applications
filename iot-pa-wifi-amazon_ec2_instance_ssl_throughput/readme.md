...
4. Run an OpenSSL server on port number 5001.
  ...
  The OpenSSL server starts running on the Amazon EC2 cloud.
...
During the data transfer, if the TCP buffer size that is sent in one chunk (BUFF_SIZE) is more than 1000 bytes, 0x5A error code occurs.
