...
int SBC_Encoder(SBC_ENC_PARAMS *pstrEncParams)
{
    ...
    return (pstrEncParams->s16NumOfSubBands * pstrEncParams->s16NumOfBlocks * pstrEncParams->s16NumOfChannels * 2);
...
* RETURNS : N/A
*/
int SBC_Encoder_Init(SBC_ENC_PARAMS *pstrEncParams)
{
        ...
        ||  (pstrEncParams->s16ChannelMode == SBC_STEREO) )
    {
        ...
        pstrEncParams->u16BitRate = s16BitRate;

        if (s16BitRate > pstrEncParams->u16BitRate)
            //s16Bitpool--;

        pstrEncParams->u16BitRate = s16BitRate + 1;
